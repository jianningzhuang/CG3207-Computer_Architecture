...
instructions
		...
		RSBS R12, R11, R6 ; 0 - 100 = -100 (0xFFFFFFF9C) C flag not set   (Load and Use Hazard => Stall D at 0x0a Flush E + W to E forwarding)
		
		RSCS R12, R11, R6 ; 0 - 100 - (~0) = -101 (0xFFFFFFF9B) C flag not set (W to D forwarding)
		
		SBCS R12, R11, R6 ; 100 - 0 - (~0) = 99 (0x63) C flag set

		RSCS R12, R11, R6 ; 0 - 100 - (~1) = -100 (0xFFFFFFF9C) C flag not set
		
		ADC R13, R12, #102 ; -100 + 102 + (0) = 2 (M to E forwarding)

		TST R11, R12 ; 0x64 & 0x01  = 0x00 (W to E forwarding)
		
		TEQ R12, #2 ; 0b01 ^ 0b10 = 0b11 (W to D forwarding)
		
		LDR R12, BOTH_WIN ; R12 hold AAAABBBB
		
		STR R12, [R4] ; display AAAABBBB on SevenSeg (W to M mem-mem copy but still covered by Load and Use => Stall)
		
		LDR R12, LARGEST_VAL ; R12 = 0xFFFFFFFF
		
		MUL R13, R12, R12 ; 0XFFFFFFFF * 0XFFFFFFFF = 0x00000001 (Load and Use Stall + MCycle Stall)

		LDR R15, PC_LDR_ADDR  ; lDR PC (flush 4 stages using PCSrcE and PCWriteLDRM/W)
...
pc_load
		B jump_back  ;just to check PC changed
...
stringptr
		DCD string1			;
PC_LDR_ADDR
		DCD 0x0000084          ; Address of the halt loop
